...
namespace World::Data {
    ...
    public:
        BitsDenseView(const unsigned int bits, const unsigned int size, uint64_t* const data)
                :mMask((1ull << bits)-1ull), mData(data),
                 mBits(bits), mSize(size), mDataSize(ceil(double(size*bits)/64.0)*sizeof(uint64_t)) { // NOLINT
            AssertBetween(1, 32, bits);
        }
        ...
        void Set(const unsigned int index, const uint64_t val) noexcept {
            AssertBetween(0, mSize-1, index);
            AssertBetween(0, mMask, val);
        ...
        [[nodiscard]] unsigned int Get(const unsigned int index) noexcept {
            ...
            const auto headBitLoc = bitOffset & 63u;
            if (hByteBegin==hByteEnd) {
                return (mData[hByteBegin] >> headBitLoc) & mMask;
            }
            else {
                const auto bitsLower = 64u-headBitLoc;
    ...
    public:
        ...
        void Set(const unsigned int index, const int val) noexcept {
            switch (mFlag) {
                case 0: A<uint8_t>(index) = val;
                    return;
                case 1: A<uint16_t>(index) = val;
                    return;
                case 2: A<uint32_t>(index) = val;
            }
        }

        [[nodiscard]] int Get(const unsigned int index) const noexcept {
            switch (mFlag) {
                case 0: return A<uint8_t>(index);
                case 1: return A<uint16_t>(index);
                case 2: return A<uint32_t>(index);
            }
            return 0;
        }
        ...
        [[nodiscard]] int GetSet(const unsigned int index, const int val) noexcept {
            switch (mFlag) {
                case 0: return Exchange(A<uint8_t>(index), val);
                case 1: return Exchange(A<uint16_t>(index), val);
                case 2: return Exchange(A<uint32_t>(index), val);
            }
            return 0;
        }
