...
namespace {
    struct Version {
        constexpr Version(int a, int b, int c, int d)
                :vMajor(a), vMinor(b), vRevision(c), vBuild(d) { }
    ...
    public:
        ...
        void load();
        [[nodiscard]] size_t getCount() const noexcept { return mModules.nameMap.size(); }
        [[nodiscard]] bool isLoaded(const std::string& uri) const {
            return mModules.nameMap.find(uri)!=mModules.nameMap.end();
        }
