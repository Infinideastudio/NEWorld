...
namespace Network {
    ...
    public:
        ...
        void UShort(uint16_t v) noexcept { (UByte(v >> 8u), UByte(static_cast<uint8_t>(v))); }
        void UInt(uint32_t v) noexcept { (UShort(v >> 16u), UShort(static_cast<uint16_t>(v))); }
        void ULong(uint64_t v) noexcept { (UInt(v >> 32u), UInt(static_cast<uint32_t>(v))); }
        ...
        void Int(int32_t v) noexcept { UInt(v); }
        void Long(int64_t v) noexcept { ULong(v); }
        ...
        static int StringSize(const std::basic_string<char, std::char_traits<char>, A>& string) noexcept {
            return VarIntSize(SizeOf(string))+SizeOf(string);
        }

        template <class C>
        static int ByteArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array); }
        template <class C>
        static int UByteArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array); }

        template <class C>
        static int ShortArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*2; }
        template <class C>
        static int UShortArraySize(const C& array) noexcept {
            return VarIntSize(SizeOf(array))+SizeOf(array)*2;
        }

        template <class C>
        static int IntArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*4; }
        template <class C>
        static int UIntArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*4; }

        template <class C>
        static int LongArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*8; }
        template <class C>
        static int ULongArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*8; }

        template <class C>
        static int FloatArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*4; }
        template <class C>
        static int DoubleArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array)*8; }

        template <class C>
        static int ByteUnboundedArraySize(const C& array) noexcept { return SizeOf(array); }
        template <class C>
        static int UByteUnboundedArraySize(const C& array) noexcept { return SizeOf(array); }

        template <class C>
        static int ShortUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*2; }
        template <class C>
        static int UShortUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*2; }

        template <class C>
        static int IntUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*4; }
        template <class C>
        static int UIntUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*4; }

        template <class C>
        static int LongUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*8; }
        template <class C>
        static int ULongUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*8; }

        template <class C>
        static int FloatUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*4; }
        template <class C>
        static int DoubleUnboundedArraySize(const C& array) noexcept { return SizeOf(array)*8; }

        template <class T, class C>
        static int ArraySize(const C& array) noexcept { return VarIntSize(SizeOf(array))+SizeOf(array); }

        template <class T, class C>
        static int UnboundedArraySizeSize(const C& array) noexcept { return SizeOf(array); }

    private:
        template <class C>
        static int SizeOf(const C& c) noexcept { return static_cast<int>(c.size()); }
        uint8_t* mHead;
        Packet& mPacket;
    };
}